...
@implementation TRTCCalling {
    BOOL _isOnCalling;
    NSString *_curCallID;
    ErrorCallback _enterRoomCallback;
}
...
- (void)call:(NSArray *)userIDs groupID:(NSString *)groupID type:(CallType)type {
    self.callingType = TRTCCallingTypeCall;
    if (!self.isOnCalling) {
         ...
         callback:(ErrorCallback)callback
{
    ...
    [self login:sdkAppId
        ...
        self.callingType = TRTCCallingTypeChatRoom;
        [self enterRoom];
...
- (NSArray<NSString *> *)getUserInfoList
{
    ...
    [list addObjectsFromArray:self.curRoomList];
    [list addObject:self.mUserID];
    return list;
}

- (NSString *)loginUserId
{
    return [[V2TIMManager sharedInstance] getLoginUser] ?: self.mUserID;
}
...
//    [beauty setBeautyLevel:6];
    ...
    param.role = TRTCRoleAnchor; // 主播角色
    param.streamId = [NSString stringWithFormat:@"%u_%u_%@_main", (unsigned int)self.mSDKAppID, (unsigned int)self.curRoomID, self.mUserID];
    ...
    [[TRTCCloud sharedInstance] enterRoom:param appScene:self.curType == CallType_Video ? TRTCAppSceneVideoCall : TRTCAppSceneAudioCall];
...
{
    [[TRTCCloud sharedInstance] muteAllRemoteAudio:isMute];
}
