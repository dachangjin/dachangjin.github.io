...
- (NSString *)invite:(NSString *)receiver action:(CallAction)action model:(CallModel *)model {
    ...
    NSMutableDictionary *param = [NSMutableDictionary dictionaryWithObjectsAndKeys:
                                  ...
                                  @(realModel.calltype),
                                  SIGNALING_EXTRA_KEY_CALL_TYPE,nil];
    ...
    case CallAction_Call:
        {
            ...
            if (isGroup) {
                ...
                @weakify(self)
                callID = [[V2TIMManager sharedInstance] invite:realModel.invitedList.firstObject
                                                          data:data
                                                       timeout:SIGNALING_EXTRA_KEY_TIME_OUT succ:^{
                    @strongify(self)
                    // 发起 Apns 推送
                    [self sendAPNsForCall:realModel.invitedList.firstObject
                              inviteeList:realModel.invitedList callID:self.callID
                                  groupid:realModel.groupid
                                   roomid:realModel.roomid];
                } fail:^(int code, NSString *desc) {
                    @strongify(self)
    ...
    case CallAction_Accept:
        {
            NSString *data = [TRTCCallingUtils dictionary2JsonStr:param];
            [[V2TIMManager sharedInstance] accept:realModel.callid
                                             data:data
                                             succ:nil
                                             fail:^(int code, NSString *desc) {
                if ([self canDelegateRespondMethod:@selector(onError:msg:)]) {
    ...
    case CallAction_Reject:
        {
            NSString *data = [TRTCCallingUtils dictionary2JsonStr:param];
            [[V2TIMManager sharedInstance] reject:realModel.callid
                                             data:data
                                             succ:nil
                                             fail:^(int code, NSString *desc) {
                if ([self canDelegateRespondMethod:@selector(onError:msg:)]) {
    ...
    case CallAction_Cancel:
        {
            NSString *data = [TRTCCallingUtils dictionary2JsonStr:param];
            [[V2TIMManager sharedInstance] cancel:realModel.callid
                                             data:data
                                             succ:nil
                                             fail:^(int code, NSString *desc) {
                if ([self canDelegateRespondMethod:@selector(onError:msg:)]) {
...
- (void)sendAPNsForCall:(NSString *)receiver
    ...
    V2TIMMessage *msg = [[V2TIMManager sharedInstance] createCustomMessage:[TRTCCallingUtils dictionary2JsonData:@{
        @"version" : @(Version)
        , @"businessID" : @"av_call"
    ...
    [[V2TIMManager sharedInstance] sendMessage:msg
                                      receiver:receiver
                                       groupID:nil
                                      priority:V2TIM_PRIORITY_HIGH onlineUserOnly:YES
...
-(void)onInvitationTimeout:(NSString *)inviteID
               inviteeList:(NSArray<NSString *> *)invitedList {
    CallModel *model = [[CallModel alloc] init];
    model.callid = inviteID;
...
- (void)handleCallModel:(NSString *)user model:(CallModel *)model {
    ...
    case CallAction_Call:
        {
            ...
            if (model.groupid != nil && ![model.invitedList
                                          containsObject:[TRTCCallingUtils loginUser]]
                ) { //群聊但是邀请不包含自己不处理
            ...
            if (self.isOnCalling) { // tell busy
                if (![model.callid isEqualToString:self.curCallID]) {
                    [self invite:model.groupid.length > 0 ? model.groupid : user
                          action:CallAction_Linebusy model:model];
                }
            } else {
                ...
                if ([self canDelegateRespondMethod:@selector(onInvited:userIds:isFromGroup:callType:)]) {
                    [self.delegate onInvited:user userIds:model.invitedList
                                 isFromGroup:self.curGroupID.length > 0 ? YES : NO callType:model.calltype];
                }
            }
    ...
    case CallAction_Timeout:
        if ([self.curCallID isEqualToString:model.callid] && self.delegate) {
            ...
            if ([model.invitedList containsObject:[TRTCCallingUtils loginUser]] &&
                self.delegate) {
                ...
                for (NSString *userID in model.invitedList) {
                    if ([self.curInvitingList containsObject:userID] &&
                        ![self.curRespList containsObject:userID]) {
...
//自动挂断
- (void)autoHangUp {
    if (self.callingType == TRTCCallingTypeChatRoom) {
        return;
    }
    [self quitRoom];
    self.isOnCalling = NO;
