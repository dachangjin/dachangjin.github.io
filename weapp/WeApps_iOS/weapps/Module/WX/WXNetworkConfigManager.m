...
const NSString *kAccessTokenName = @"access_token";
const NSString *kCheckTokenName = @"check_token";
const NSString *kRefreshTokenName = @"refresh_token";
const NSString *kUserinfoName = @"getuserinfo";

static NSString* const kConfigureItemsKeyName = @"kConfigureItemsKeyName";



@interface WXNetworkConfigManager()

@property (nonatomic, strong) NSMutableDictionary *allConfig;

@end

@implementation WXNetworkConfigManager

#pragma mark - Life Cycle
+ (instancetype)sharedManager {
    static dispatch_once_t onceToken;
    static WXNetworkConfigManager *instance = nil;
    dispatch_once(&onceToken, ^{
        instance = [[WXNetworkConfigManager alloc] initInPrivate];
    });
    return instance;
}
...
- (void)setup {
    ...
    if ([[self.allConfig allKeys] count] != [json count]) {
        ...
        for (NSDictionary *dict in json) {
            WXNetworkConfigItem *item = [WXNetworkConfigItem modelObjectWithDictionary:dict];
...
- (void)registerConfig:(WXNetworkConfigItem *)item forKeyPath:(NSString *)keyPath {
...
- (WXNetworkConfigItem *)getConfigForKeyPath:(NSString *)keyPath {
